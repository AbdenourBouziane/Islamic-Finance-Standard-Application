  ...
  Future<String> getExplanation(String standardId, String scenario) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return isEnglish 
          ? 'Failed to get explanation. Please try again later. Status: ${response.statusCode}'
          : 'فشل في الحصول على الشرح. يرجى المحاولة مرة أخرى لاحقًا. الحالة: ${response.statusCode}';
      }
    } catch (e) {
      print('Exception getting explanation: $e');
      return isEnglish 
        ? 'Network error. Please check your connection. Error: $e'
        : 'خطأ في الشبكة. يرجى التحقق من اتصالك. الخطأ: $e';
    }
  }
  ...
  Future<Map<String, dynamic>> getFeedback(String standardId, String userSolution) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return {
          'feedback': isEnglish 
            ? 'Failed to get feedback. Please try again later. Status: ${response.statusCode}'
            : 'فشل في الحصول على التعليقات. يرجى المحاولة مرة أخرى لاحقًا. الحالة: ${response.statusCode}',
          'expert_solution': '',
        };
      }
    } catch (e) {
  ...
  Future<String> askCustomQuestion(String question) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return isEnglish 
          ? 'Failed to get answer. Please try again later. Status: ${response.statusCode}'
          : 'فشل في الحصول على الإجابة. يرجى المحاولة مرة أخرى لاحقًا. الحالة: ${response.statusCode}';
      }
    } catch (e) {
